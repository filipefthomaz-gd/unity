...
#include "UnityCG.cginc"
    ...
    v2f vert(appdata v)
    {
        ...
        float4 modifiedPos = o.vertex;
        modifiedPos.x += sin(v.vertex.y * 100 + _Time.y * 2) * 0.005;
...
#else  
        noise += gradientBlend;// fade the object at the top
#endif
#ifdef SHAPEX_ON
        noise += gradientBlend;// fade the object at the top over mask
#endif
        float4 object = saturate(noise.a * _Hard); //noise object
        float4 objectcolored =object *gradientMain; // coloured noise object
        float4 rim = saturate((noise.a + _Edge) * _Hard) - object; // noise object edge
 
        float4 objectcolored2 = rim * gradientTint; // coloured object edge
        float4 finalcolor = objectcolored + objectcolored2; // combined edge and object
        return finalcolor;
