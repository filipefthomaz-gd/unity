...
Shader "FX/Mirror"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 _MainTex_ST;
			fixed4 _Color;
            float4 _DistortionScrollX;
            float4 _DistortionScrollY;
            float4 _RefractionX;
            float4 _RefractionY;
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				tex.rgb *= tex.a;
				fixed4 refl = tex2Dproj(_ReflectionTex, UNITY_PROJ_COORD(i.refl + 0.01*(tex2D(_NoiseTex, distortionScale * i.refl))));
				return tex * refl;
			}
