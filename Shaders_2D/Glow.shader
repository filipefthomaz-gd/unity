...
//Kaan Yamanyar,Levent Seckin
 Shader "Sprites/ShinyDefault"
 {
     Properties
     {
         ...
         _XOrientation("XOrientation", Range(-1,1)) = 1
         _YOrientation("YOrientation", Range(-1,1)) = 1
 ...
 #include "UnityCG.cginc"
     ...
     fixed4 SampleSpriteTexture(float2 uv)
     {
         ...
         float currentDistanceProjection = (uv.x*_XOrientation + uv.y*_YOrientation) / 2;
         if (currentDistanceProjection > lowLevel && currentDistanceProjection < highLevel) {
             ...
             color.rgb +=  color.a * whitePower * _Strength;
         }
